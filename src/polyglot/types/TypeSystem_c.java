...
public class TypeSystem_c implements TypeSystem {
	...
	/**
	 * Checks whether the member mi of container container can be accessed from
	 * code that is declared in the class contextClass.
	 */
	@Override
	...
	protected Set<FieldInstance> findFields(ReferenceType container, String name) {
		...
		if (container.superType() != null
			...
			Set<FieldInstance> superFields = findFields(container.superType()
					.toReference(), name);
		...
		if (container.isClass()) {
			...
			for (Type it : ct.interfaces()) {
				Set<FieldInstance> superFields = findFields(it.toReference(),
						name);
				fields.addAll(superFields);
			}
		}
	...
	public MethodInstance findMethod(ReferenceType container, String name,
		...
		List<? extends MethodInstance> acceptable = findAcceptableMethods(
				container, name, argTypes, currClass);
	...
	public ConstructorInstance findConstructor(ClassType container,
		...
		List<? extends ConstructorInstance> acceptable = findAcceptableConstructors(
				container, argTypes, currClass);
	...
	protected <Instance extends ProcedureInstance> Collection<Instance> findMostSpecificProcedures(
		...
		acceptable = new ArrayList<Instance>(acceptable); // make into array
															// list to sort
	...
	public boolean methodCallValid(MethodInstance prototype, String name,
		...
		assert_(prototype);
		assert_(argTypes);
		boolean flag = prototype.methodCallValidImpl(name, argTypes);
		return flag;
	}
	...
	public MethodInstance findImplementingMethod(ClassType ct, MethodInstance mi) {
		...
		while (curr != null) {
			List<? extends MethodInstance> possible = curr.methods(mi.name(),
					mi.formalTypes());
